...
export default function CourseSection({ videoData, setVideoUrls }: Props) {
  ...
  // Auto-select first video of the first section when data arrives
  useEffect(() => {
    if (videoData?.course?.sections?.[0]?.episodes?.[0]) {
      const firstEp = videoData.course.sections[0].episodes[0];
  ...
  if (!videoData?.course?.sections) {
    return null
  }

  return (
    <div className="rounded border border-gray-200 overflow-hidden">
      {videoData.course.sections.map((section) => {
        const episodes = section.episodes || [];
        const totalCount = episodes.length;
        const selectedIndex = episodes.findIndex((ep) => ep.id === selectedId);
        const totalDuration = formatMinutes(episodes);
                ...
                return (
                  <li key={ep.id}>
                    <button
                      onClick={() => {
                        setSelectedId(ep.id);
                        setVideoUrls([ep.video_url]); //  update parent to play this video
