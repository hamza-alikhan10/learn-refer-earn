...
const ReferralModal: React.FC = () => {
  ...
  // If modal is closed or no data, render nothing
  if (!isOpen || !course || !referral) return null;
  ...
  const handleShare = (platform: string) => {
    const message = `Check out this amazing course: ${course.title} on AffiliateHub! Use my link: ${referralLink}`;
    switch (platform) {
      case "email":
        window.open(
          `mailto:?subject=Check out this course&body=${encodeURIComponent(
            message
          )}`
        );
        break;
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="flex rounded-lg overflow-hidden border border-input">
              ...
              >
                {copied ? (
                  <Check className="w-4 h-4 icon-3d" />
                ) : (
                  <Copy className="w-4 h-4 icon-3d" />
                )}
              </Button>
            </div>
            {copied && (
